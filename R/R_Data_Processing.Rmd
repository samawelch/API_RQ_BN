...
# Import in Joanke's API removal rates
API_Removal_Rate <- read_excel(path = "Data/Removal_efficiencies_JvD.xlsx",
                               col_names = c("API_Name",
                                             "Source",
                                             "ATC",
                                             "CAS",
                                             "SMILES",
...
```{r removal_rate_cleaning}
API_Removal_Rate_Clean <- API_Removal_Rate %>% 
  dplyr::select(API_Name, Primary, Secondary, Advanced)
```
...
Hugin_Data_File <- LMs_API %>% 
  ...
  crossing(Population_Scenario = c("Low", "Main", "High"),
           ...
           WWTP_Removal_Scenario = c("Low", "Moderate", "High")) %>% 
...
Hugin_Data_Output <- read_csv(file = "Data/Hugin/Hugin_Paracetamol_Output.csv",
...
Diclofenac_Weird <- Hugin_Data_Output %>% 
  ...
  dplyr::select(API_Name, Year, API_Sales_Weight_kg, Population_Scenario) %>% 
  distinct()
