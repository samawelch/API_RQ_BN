...
# What does the original data look lile?
...
```{r Hugin_Data_Maker}
# Pick three APIs with high/medium/low sales weights
three_APIs <- c("paracetamol", "diclofenac", "levonorgestrel")

# Make data files for automated data input/output to Hugin
Hugin_Data_File <- LMs_API %>% 
  left_join(API_PNECs, by = "API_Name") %>% 
  transmute(API_Name,
            API_LM_Intercept_g = lm_intercept_g,
            API_LM_Coeff_g_KPop = lm_coef_g_kpop,
            API_LM_Coeff_g_Year = lm_coef_g_year,
            API_PNEC_gL = PNEC_gL) %>% 
  ...
  crossing(Population_Scenario = as_factor(c("Low", "Main", "High")),
           Year = c("2010", "2020", "2030", "2040", "2050"),
...
RQ_by_Year_Scen_API <- ggplot(data = Hugin_Data_Output,
       mapping = aes(x = Year,
                     y = `[MEAN](RQ)`,
                     colour = as.factor(WWTP_Removal_Scenario),
                     shape = Population_Scenario)) +
  geom_point() +
  geom_path() +
...
```{r thinking_about_parameterisation}
