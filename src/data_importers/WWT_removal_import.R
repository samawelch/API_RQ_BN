...
API_removal_rates <- read_excel(
  path = "data/raw//Removal_efficiencies_JvD.xlsx",
  range = "A1:M59"
) %>%
  transmute(
...
API_removal_rates <- API_removal_rates %>% add_row(
  ...
  API_removal_rates %>%
    group_by(treatment) %>%
    reframe(
      removal_rate_perc = mean(removal_rate_perc, na.rm = TRUE),
      treatment,
...
API_removal_rates_simple <- API_removal_rates %>%
  mutate(treatment = case_when(
    treatment %in% c("secondary_removal", "tertiary_removal") ~ "sec_tert_removal",
    TRUE ~ treatment
  )) %>%
  group_by(treatment)
