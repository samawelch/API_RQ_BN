...
pop_by_county_2020 <- read_excel(
  path = "data/raw/Statistics_Norway/Pop_2020_Counties.xlsx",
  range = "A5:B15",
...
WWT_definitions_Norway_EU <- WWT_definitions_Norway |>
  select(Name_EN, Class_EU) |>
  distinct()

### SSB: Inhabitants Connected to Small WWTPs (05272)
# https://www.ssb.no/statbank/table/05272/
small_wwtp_by_county_2020 <- read_excel(
  path = "data/raw/Statistics_Norway/WWTP_by_County_2020_Small.xlsx",
  range = "A6:P16",
  col_names = c(
    "County",
    "total",
  ...
  mutate(
    County_Code = str_extract(string = County, pattern = "[0-9]{2}"),
...
large_wwtp_by_county_2020 <- read_excel(
  path = "data/raw/Statistics_Norway/WWTP_by_County_2020_Large.xlsx",
  range = "A6:H16",
  col_names = c(
    "County",
    "total",
...
wwtp_by_county_2020 <- large_wwtp_by_county_2020 |>
  ...
  left_join(WWT_definitions_Norway_EU, by = c("treatment" = "Name_EN")) |>
...
Norway_Population_Year <- read_xlsx(
  path = "data/raw/Statistics_Norway/Pop_1951_2021.xlsx",
  range = "B4:C74",
...
Norway_Population_Projections_21C <- read_xlsx(
  path = "data/raw/Statistics_Norway/Pop_Forecasts_21C.xlsx",
  range = "B5:J7",
  col_names = c(
...
Norway_County_Urbanisation_2020 <- read_xlsx(
  path = "data/raw/Statistics_Norway/Norway_Pop_Urban.xlsx",
  range = "A6:C16",
...
Norway_County_General_2020 <- Norway_County_Urbanisation_2020 |>
  ...
  select(-quantile) |>
  mutate(exemplar = case_when(
    County_Name %in% c("Viken", "Trøndelag", "Troms & Finnmark") ~ TRUE,
    TRUE ~ FALSE
  )) |>
